...
# Função unique() = Lê um arquivo e retorna apenas uma ocorrência dos dados.
# Aqui ela vai formar as caixas de seleção, sem repetir as mesmas opções.
...
server <- function(input, output) {
    observeEvent(input$processar, {
        novocarro = data.frame("buying"=input$buying, "maint"=input$maint, "doors"=input$doors,
                               "persons"=input$persons, "lug_boot"=input$lug_boot, "safety"=input$safety)
